...
FROM 
	医薬品
WHERE 
	診療年月 BETWEEN 201710 AND 201801
AND 
	剤型大分類名 IN ('内用薬', '注射薬')
AND 
...
DROP TABLE IF EXISTS JMDC_ARM_SQLdata_1month;
CREATE TABLE JMDC_ARM_SQLdata_1month 
SELECT 
...
FROM 
	傷病_sub
;
ALTER TABLE JMDC_ARM_SQLdata_1month ADD INDEX index01(加入者ID); 
ALTER TABLE JMDC_ARM_SQLdata_1month ADD INDEX index02(Item);
